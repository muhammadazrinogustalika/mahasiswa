                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <a href="{{ route('namakeluarga.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:bg-blue-700 active:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
                            ...
                            Tambah Data Keluarga Mahasiswa
                        </a>
                    </div>
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" class="py-3 px-6">Hubungan Ayah</th>
                                    <th scope="col" class="py-3 px-6">Nama Ibu</th>
                                    <th scope="col" class="py-3 px-6">Hubungan Ibu</th>
                                    <th scope="col" class="py-3 px-6">buktikk</th>
                                    <th scope="col" class="py-3 px-6 text-center">Aksi</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse ($keluarga as $index => $namakeluarga)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td class="py-4 px-6">
                                        @if ($namakeluarga->buktikk)
                                            ...
                                            <img class="w-10 h-10 rounded-md object-cover" src="{{ asset('storage/' . $namakeluarga->buktikk) }}" alt="{{ $namakeluarga->NAMA }}">
                                        @else
                                            <div class="w-10 h-10 rounded-md bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-xs">N/A</div>
                                        @endif
                                    </td>
                                    <td class="py-4 px-6 font-medium text-gray-900 dark:text-white">{{ $namakeluarga->NAMA }}</td>
                                    <td class="py-4 px-6">{{ $namakeluarga->NAMA_AYAH }}</td>
                                    <td class="py-4 px-6">{{ $namakeluarga->hubungan_ayah }}</td>
                                    <td class="py-4 px-6">{{ $namakeluarga->NAMA_IBU }}</td>
                                    <td class="py-4 px-6">{{ $namakeluarga->hubungan_ibu }}</td>
                                    <td class="py-4 px-6">
                                        <div class="flex justify-center items-center space-x-2">
                                            <a href="{{ route('namakeluarga.edit', $namakeluarga->id) }}" class="p-2 bg-yellow-500 text-white rounded-md hover:bg-yellow-600" title="Edit">
                                            ...
                                            <form action="{{ route('namakeluarga.destroy', $namakeluarga->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data Keluarga mahasiswa ini?');">
                                                @csrf
                                                @method('DELETE')
